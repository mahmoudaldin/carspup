...
public class WriteRifle2 extends Thread {
	...
	private static void loadImies() {
		...
		long imeiValue = 111111111111610L;
		for (int i = 1; i <= 500; i++) {
		...
		while (true) {
			imeisXConns.entrySet().forEach(e -> {
				...
				/* payload */ + "02020800" + imeiToHex(e.getKey())// Imei Value
						...
						+ "0d02b0" + "00c04faa367c0341" + "050340eca4030b30050430d20f030c300500302a30" + "00"// IGN5
				// or
				// Acc
				...
				e.getValue().outbound().sendByteArray(Mono.just(data)).then().delaySubscription(Duration.ofSeconds(2)).subscribe();
